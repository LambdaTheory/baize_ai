...
class PromptTag(QWidget):
    ...
    def _init_ui(self):
        ...
        if (
            self.chinese_text
            and self.chinese_text.strip()
            and self.chinese_text.strip() != self.english_text.strip()
        ):
    ...
    def _update_style(self):
        ...
        if self.is_selected:
            ...
            self.setStyleSheet(
                """
                QWidget#prompt_tag {
                    background-color: #656565; /* 直接使用黑色背景 */
                    border: 1px solid #333;
                    border-radius: 6px;
                }
                QLabel#english_label {
                    color: white;
                    background-color: #656565; /* 透明，透出父级的黑色 */
                    padding: 6px 10px;
                    border-top-left-radius: 5px;
                    border-bottom-left-radius: 5px;
                }
                QLabel#chinese_label {
                    background: qlineargradient(x1:0, y1:0, x2:0, y2:1, stop:0 #a0b181, stop:1 #57b049); /* 使用Qt特定的渐变语法 */
                    color: white;
                    padding: 6px 10px;
...
class AccordionCard(CardWidget):
    ...
    def setExpanded(self, expanded):
        ...
        if self._expanded:
            ...
            target_height = (
                self._content_widget.sizeHint().height() + 16
                if self._content_widget
                else 200
            )
